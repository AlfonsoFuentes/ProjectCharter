...
public partial class ProjectsTable
{
    ...
    public List<ProjectResponse> Items = new();
    public List<ProjectResponse> FilteredItems => Items.Count == 0 ? new() :
        _selectedProjectId == Guid.Empty ? Items : ProjectList.Items.Where(x => x.Id == _selectedProjectId).ToList();
    ...
    void onValueSearchChanged(string value)
    {
        nameFilter = value;
        ...
        {
            Items = ProjectList.Items.Where(x =>
               x.Status.Name.Contains(nameFilter, StringComparison.OrdinalIgnoreCase) ||
            x.Name.Contains(nameFilter, StringComparison.OrdinalIgnoreCase) ||
              x.ProjectNumber.Contains(nameFilter, StringComparison.OrdinalIgnoreCase) ||
                x.Focus.Name.Contains(nameFilter, StringComparison.OrdinalIgnoreCase)
            ).ToList();
        }
